...
# Check for FFT availability
try:
    from scipy.fft import fft, fftfreq
    HAVE_FFT = True
except ImportError:
    HAVE_FFT = False
    print("⚠ SciPy FFT not available - skipping frequency spectrum plots")
...
def test_baseline_removal():
    ...
    # Create figure - adjust layout based on FFT availability
    if HAVE_FFT:
    ...
    for i, method in enumerate(methods):
        try:
            ...
            # Plot 4: Frequency spectrum comparison (only if FFT available)
            if HAVE_FFT:
                ax4 = axes[i, 3]
                # Compute FFT
                n = len(corrupted_ecg)
            ...
            col_count = 4 if HAVE_FFT else 3
